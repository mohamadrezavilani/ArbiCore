...
async def force_rebalance(symbol: str, db: AsyncSession = Depends(get_db)):
    ...
    Returns a detailed reason for success or failure.
    """
    ...
    # Create rebalancer (simulator mode will be auto-detected)
    rebalancer = Rebalancer(OpportunityLogger(), TradeExecutor(OpportunityLogger()))

    # Call rebalancer - now returns (success, reason)
    success, reason = await rebalancer.rebalance_symbol_if_needed(
        db, symbol, "IRT", exchange_orderbooks[symbol]
    )

    # Commit any changes made by rebalancer (it commits inside, but safe to commit again)
    await db.commit()
