...
class OrderResult:
    ...
    def __post_init__(self):
        # If executions are provided, compute filled_price and filled_volume from them
        if self.executions:
        ...
        # Otherwise keep the passed values
...
class ExchangeClient(ABC):
    ...
    async def place_market_order(self, symbol: str, side: str, amount: float, client_order_id: str) -> OrderResult:
        """
        Place a LIMIT order (we treat it as "market" but with specified price).
        The price is determined from the best available.
        """
        pass
